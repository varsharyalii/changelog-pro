...
class ChangelogService {
  constructor(options = {}) {
    ...
    this.config = {
      ...
      template: 'default', // Default to default template
      ...options.config
    };
  }
  ...
  async processChangelog() {
    try {
      ...
      // Step 3: Render HTML with template name and options
      const packageInfo = this.getPackageInfo();
      const newHtml = this.htmlRenderer.render(releases, this.config.template, {
        title: packageInfo.name || 'Changelog',
        description: `Changelog for ${packageInfo.name || 'this project'}`,
        author: packageInfo.author || 'Development Team'
      });

      // Step 4: Write output
      fs.writeFileSync(this.config.htmlFile, newHtml);
  ...
  shouldUseNewTemplate() {
    // Use new template (HtmlFormatter) only if template is explicitly set to 'professional'
