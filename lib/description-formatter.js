...
class DescriptionFormatter {
  static SECTION_PRIORITIES = ['breaking', 'features', 'fixes', 'improvements', 'security'];
  ...
  formatReleaseDescriptions(release) {
    ...
    return descriptions.length > 0 ? descriptions : ['Various improvements and bug fixes'];
  }
  ...
  _formatSectionDescription(sectionType, items) {
    ...
    const countLabel = sectionType === 'breaking' ? 'Breaking changes' : `${count} ${sectionType}`;
